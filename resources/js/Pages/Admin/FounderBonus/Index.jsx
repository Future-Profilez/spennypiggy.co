                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500 font-medium">Track qualifications, seats, and founder payout settlements</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <Link
                                href="/founder/bonus"
                                className="px-4 py-2 text-xs font-bold uppercase tracking-wider text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors"
                            >
                                Public Leaderboard ↗
                            </Link>
                        </div>
                    ...
                    {stats && (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                                <div className="text-[11px] font-black uppercase tracking-wider text-gray-400">Total Founders</div>
                                <div className="text-2xl font-black text-gray-900 mt-1">{stats.total_founders || 0}</div>
                                <div className="text-xs text-gray-500 mt-1 font-medium">{stats.seats_remaining || 0} of {stats.max_seats || 150} seats left</div>
                            </div>
                            <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                                <div className="text-[11px] font-black uppercase tracking-wider text-gray-400">Pending Payouts</div>
                            ...
                            <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                                <div className="text-[11px] font-black uppercase tracking-wider text-gray-400">Total Paid</div>
                            ...
                            <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                                <div className="text-[11px] font-black uppercase tracking-wider text-gray-400">Qualification Target</div>
                                <div className="text-2xl font-black text-gray-900 mt-1">£{Number(stats.min_earnings || 2500).toLocaleString()}</div>
                                <div className="text-xs text-gray-500 mt-1 font-medium">{stats.bonus_percentage || 10}% bonus rate</div>
                            </div>
                        </div>
                    )}
                        ...
                        {recentBonuses && recentBonuses.length > 0 ? (
                            <div className="overflow-x-auto">
                                <table className="w-full text-left text-sm">
                                    <thead className="bg-gray-50 text-gray-500 uppercase text-[11px] font-black tracking-wider rounded-xl">
                                        <tr>
                                        ...
                                        {recentBonuses.map((b) => (
                                            <tr key={b.id} className="hover:bg-gray-50/80 transition-colors">
                                                <td className="px-4 py-3">
                                                    ...
                                                    <div className="text-xs text-gray-500">{b.creator_email}</div>
                                                </td>
