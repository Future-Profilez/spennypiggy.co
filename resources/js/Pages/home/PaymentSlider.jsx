...
const logos = [
    { svg: <><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="50" height="50" viewBox="0 0 50 50">
...
export default function PaymentSlider() {
  /* The track renders from `logos` rather than from hand-written slides.
     There were 18 <SwiperSlide> elements for 5 logos — the same set pasted three
     times — and only 6 carried an alt. That duplication is why removing one logo
  ...
  return (
    <div className="py-8 md:py-10">
      ...
      >
        ...
        >
          {track.map((logo, i) => (
            <SwiperSlide key={`${logo.alt}-${i}`} className="!w-auto">
              {/* Brutalist: solid fill, hard border, no offset shadow. */}
              <div className="flex h-[54px] min-w-[96px] items-center justify-center rounded-box-sm border-2 border-black bg-white px-4 md:h-[64px] md:min-w-[118px] md:px-6">
                {logo.svg ? (
                  <span className="flex h-8 w-auto items-center justify-center md:h-9 [&>svg]:h-full [&>svg]:w-auto">
                    {logo.svg}
                  </span>
                ) : (
                  <img
                    ...
                    loading="lazy"
                    decoding="async"
                    className="h-7 w-auto max-w-[76px] object-contain md:h-8 md:max-w-[88px]"
                  />
                )}
              </div>
            </SwiperSlide>
