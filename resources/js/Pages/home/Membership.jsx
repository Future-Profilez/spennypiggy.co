    ...
    <section id='features' className="bg-black relative overflow-hidden py-24 px-4 text-center">
      ...
      <div className="containerbox relative  ">
        ...
        <h2 className="text-2xl md:text-3xl lg:text-5xl font-gulfs text-white text-center mb-16 uppercase leading-none tracking-[2px] max-w-6xl mx-auto drop-shadow-2xl">
            <span className="block mb-2 drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]">Supporting Creators!</span>
            <span className="block mb-2 text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-pink-500 to-purple-500 animate-pulse drop-shadow-none">Empowering Gifters!</span>
            <span className="block text-yellow-400 drop-shadow-[4px_4px_0_rgba(0,0,0,1)]" style={{WebkitTextStroke: '1px #000'}}>Made for Everyone!</span>
        </h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8 max-w-7xl mx-auto">
          {supportData.map((item, index) => (
            ...
            <h3 className="mt-6 leading-tight text-lg md:text-xl uppercase font-gulfs text-white group-hover:text-pink-400 transition-colors tracking-wide">
              {item.title}
            </h3>

            <p className=" text-sm text-gray-400 leading-relaxed max-w-[16rem] font-poppins group-hover:text-gray-200 transition-colors">
              {item.description}
            </p>
          </div>
          ))}
