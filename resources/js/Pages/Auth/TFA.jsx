            ...
            <Popup space="4" action={open}
                ...
                {isTFA == 0 ?
                        <>
                            {backupCodes && backupCodes.length ? <>
                                <div className="backcodes">
                                    ...
                                    <div className="codes bg-gray-100 p-4 rounded-box   relative">
                                        {backupCodes && backupCodes.map((code, index) => (
                                            <p key={index} className="mb-2 text-lg">{code}</p>
                                        ))}
                                        <button className="absolute top-2 right-3 text-[14px] bg-gray-200 px-3 py-1 rounded-box   " onClick={()=>copyCodes(backupCodes)} >Copy</button>
                                    </div>
                                </div>
                                </>
                                :
                                <>
                                    <div className="enableTFA">
                                        ...
                                        <div className={`step1 ${step === 2 ? "visible" : "hidden"}`}>
                                            ...
                                            <div
                                                className="rounded-box   max-w-[200px] mx-auto block my-3"
                                                ...
                                                <div className="flex items-center mt-4">
                                                    {otp.map((data, index) => (
                                                        <input
                                                            key={index}
                                                            type="text"
                                                            className="border-gray-300  text-center bg-gray-200 text-black rounded-box   w-full max-w-[40px] min-h-[40px] otp-input mx-1 px-1 py-1 "
                                                            maxLength="1"
                                                            value={data}
                                            ...
                                            <button onClick={verify}
                                                className="border-0 pinkbg rounded-box    px-3 py-2 text-lg text-black mx-auto block w-full mt-4 ">{verifying ? "VERIFYING..." : "VERIFY"}
                                            </button>
                                            {/* <button className="text-center text-[#FF007F] my-3 cursor-pointer w-3/4 mx-auto block font-normal">Verify with backup code.</button> */}
                                        </div>
                                    </div>
                                </>
                            }
                    ...
                    <div className="backcodes">
                        <h2 className="font-bold text-xl mb-1">Two Factor Authentication</h2>
                        <p className="text-gray-700 mb-4"> Two-step verification adds an extra layer of protection to your account. After you've turned it on,we'll ask you to enter an additional security code when you sign in. We'll provide this security code only to you. </p>
                            <button onClick={disable2fa} className="bg-red-600 text-white m-auto px-3 py-2 rounded-box  " >Disable 2FA</button>
                     </div>
                }
            </Popup>
