...
export default function Allwishes(props) {
    ...
    const fetch_wishes = (p) => {
        setLoading(true);
        axios.get(`discover/wishes/${order}/${type}/${price}?page=${p}`).then((resp) => {
    ...
    const Switch = () => { 
        return <div className='d-flex align-items-center toggleswitch mb-3 mb-sm-0' >
    ...
    const Pagination = ({ number }) => {
        ...
        useEffect(() => {
            ...
            return <>
            {pageArray && pageArray.length > 1 ? <ul className='mt-4 pagination d-flex flex-wrap justify-content-center' >
                {pageArray && pageArray.map((p, i)=>{
                    return <li className='pe-2 mb-2' key={`page-${i}`} >
                        <button className={p == page ? 'active' : ''} onClick={()=>fetch_wishes(p)} >{p}</button>
                    </li> 
                })}
            </ul> : ''}
        </>
    ...
    return (
        ...
        <div className='filters d-block d-sm-flex align-items-center justify-content-between w-100 mb-4' >
            ...
            <div className='d-flex align-items-center' >
                ...
                <div className='filter-select-wrap' >
                    <select onChange={(e)=> setType(e.target.value)} id="types" class=" filter-select bg-gray-50 border border-gray-300 text-gray-900 
                    text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 
                    block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 
                    dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option selected value="all" >All Wishes</option>
                ...
                <div className='filter-select-wrap ps-3' >
                    <select onChange={(e)=>setprice(e.target.value)} id="prices" class="filter-select bg-gray-50 border border-gray-300 text-gray-900 
                        text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 
                        block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 
                        dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option selected value="all" >By Price</option>
        ...
        <div className='row' >
            ...
            {loading? <div className='w-100 d-flex justify-content-center' ><LoadingScreen /></div>  
            : 
            <>
                {wishes && wishes.length ? wishes.map((w, i)=> { 
                    return <Wishlistbox key={`wish-item-${i}`} classes="col-xl-3 col-lg-4 col-6"
                        currency={global_currency}
                        // fetchingcats={fetchingcats}
                        // categories={categories} 
                        IsloggedIn={false} showall={true}
                        auth={auth.user}
