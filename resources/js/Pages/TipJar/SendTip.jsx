...
export default function SendTip({auth, classes, card_capabilities}) {
    ...
    return (
        <>
            ...
            action={close} classes={`uppercase font-gulfs btn-shadow text-sm font-bold rounded-full px-6 py-2.5 bg-[#F94F97] text-white shadow-lg shadow-[#F94F97]/20 hover:scale-105 transition-all ${classes} ${auth?.user ? 'loggedin' : 'logout'} `}
            text={`Support Me`} >
