...
export default function ShopDetailItem(props) {
    ...
    const [open, setOpen] = useState(false); // Changed from undefined to false

    useEffect(() => {
        // Add null check for props.opened
        if (props.payment_id && props.opened === 0) {
            // Changed == to ===
            setOpen(true);
            if (shop && shop.success_page_type === "url") {
                // Added shop null check
                window.open(shop && shop.success_page_value, "_blank");
            }
        }
    }, [props.payment_id, props.opened, shop]); // Added proper dependencies
  ...
  useEffect(()=>{
   getIp();
    ...
    const getShippingPrice = (c) => {
        if (!shop?.uuid) return; // Added check
        axios
    ...
    useEffect(() => {
        getIp();
    }, [shop?.uuid]); // Added dependency

    // Fixed the warning by changing the breadcrumb structure
    return (
        <>
                            ...
                            <div className="product-details max-w-[700px] px-2 mx-auto">
                                ...
                                >
                                    <ol className="inline-flex flex-wrap items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                                        ...
                                        <li className="flex items-center">
                                            ...
                                            <span className="ml-1 text-base font-medium text-gray-700 md:ml-2">
                                                Shop
                                            </span>
                                        </li>
                                        <li className="flex items-center">
                                ...
                                <div className="w-full relative">
                                    <img
                                        ...
                                        src={shop?.perma_link}
                                    />
                                    {shop?.ai_generated === 1 && ( // Changed == to === and added optional chaining
