...
export default function AddItem({item, update, title, pre_title, pre_description, pre_price, product_type, classes, isEdit}) {
   ...
   const AddForm = () => { 
      ...
      const [question, setQuestion] = useState(item && item.ask_question || '');
      const [haveSlots, setHaveSlots] = useState(item && item.slot_limitation ? true : false);
      const [slots, setSlots] = useState(item && item.slot_limitation || '');
      const [haveSpPrice, setHaveSpPrice] = useState(item && item.special_member_price ? true : false);
      const [spPrice, setSpPrice] = useState(item && item.special_member_price || '');
      const [haveQty, setHaveQty] = useState(item && item.quantity_allow ? true : false);
      const [pagetype, setPageType] = useState(item && item.success_page_type || false);
      const [parsedContent, setParsedContent] = useState(item && item.success_page_value || '');
      ...
      const addCategory = () => { 
         ...
         setAdding(true);
         axios.post(`/shop/save-category`, { category: value })
            .then((res) => {
               if (res.data.status) {
      ...
      const updateItem = () => { 
         ...
         axios.post(`/shop/update/${item.uuid}`, data)
            .then((res) => {
               if (res.data.status) {
                  successAlert(res.data.msg || "Item Added !!");
                  resetUploader();
      ...
      return (
         <Popup modalclass='addShopItem modal full' size='lg' action={close} 
         text={title || 'open'}
         classes={`${classes ? classes : "px-3 py-2"}`} >
            <div className='p-3 md:p-8 overflow-auto bg-white md:bg-gray-200 h-full' >
               ...
               <div className='shop-forms-field p-0 md:p-8 max-w-[800px] m-auto rounded-[20px]' >
                  ...
                  {isEdit ? 
                  <>
                     <img alt="image-profile" className="w-full max-h-[500px] object-cover h-auto rounded-4" src={item && item.perma_link} />
                  </>
                  : ""
                  }
                     ...
                     <label className='w-full mb-2' >Success page </label>
                        <div className='success-page-types flex items-center flex-wrap' >
                           <div className="flex items-center mb-2 pe-3">
                              <input onChange={handleSuccessPageType} defaultChecked={item && item.success_page_type == "text" ? true : false}  id="success-option-1" type="radio" name="success-types" value="text" className="h-4 w-4 border-gray-300 focus:ring-2 focus:ring-blue-300 cursor-pointer" />  
                              <label htmlFor="success-option-1" className=" cursor-pointer text-md font-medium text-gray-900 ml-2 block">
                                 Confirmation message
                              </label>
                           </div>
                           <div className="flex items-center mb-2 ">
                              <input onChange={handleSuccessPageType} defaultChecked={item && item.success_page_type == "url" ? true : false} id="success-option-2" type="radio" name="success-types" value="url" className="h-4 w-4 border-gray-300 focus:ring-2 focus:ring-blue-300 cursor-pointer"  />
                        ...
                        {pagetype == "url" ? 
                           <input defaultValue={pageUrl} onChange={(e)=>setpageUrl(e.target.value)} className='mt-2 shop-forms-input bg-gray-200 w-full bg-gray-200 border-0 rounded-xl p-3 px-3.5' type="text" placeholder='https://' /> : ""
                        }
                  </div>
                  ...
                  <h2 className='text-lg font-bold mb-2 border-t pt-3 mt-4' >Advanced Settings</h2>
                  ...
                  <div className='ad-setting my-2' >
                     ...
                     {haveQuestion ? <input defaultValue={item && item.ask_question} onChange={(e)=>setQuestion(e.target.value)} className='mt-2 mb-3 shop-forms-input bg-gray-200 w-full bg-gray-200 border-0 rounded-xl p-[13px] px-4' type="text" placeholder='e.g What would like to learn next ?' /> : ''}
                  </div>
                  ...
                  <div className='ad-setting my-2' >
                     ...
                     {haveSlots ? <input onChange={(e)=>setSlots(e.target.value)} className='mt-2 mb-3 shop-forms-input bg-gray-200 w-full bg-gray-200 border-0 rounded-xl p-[13px] px-4' type="text" defaultValue={slots || ''}  /> : ''}
                  </div>
                  ...
                  <div className='ad-setting my-2' >
                     ...
                     {haveSpPrice ? <input onChange={(e)=>setSpPrice(e.target.value)} className='mt-2 mb-3 shop-forms-input bg-gray-200 w-full bg-gray-200 border-0 rounded-xl p-[13px] px-4' type="text" defaultValue={spPrice || ''}  /> : ''}
                  </div>
                  ...
                  <div className='ad-setting my-2' >
                     <div className="inline-flex items-centercursor-pointer">
                        ...
                        <span className="ms-3 text-md font-medium text-gray-900">Allow buyer to choose a quantity (optional) <button className='tooltipbtn' >?<p>Your supporters will be able to select the desired quantity of this item && item. You will receive payment based on the quantity they choose multiplied by your set price.</p></button></span>
                     </div>
                  </div>
