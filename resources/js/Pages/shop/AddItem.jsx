...
export default function AddItem(props) {
    ...
    const [wwsShipping, setwwsShipping] = useState(() => {
        if (item && item.shop_shipping_info) {
            const wws = item.shop_shipping_info.find(
                (s) => s.country === "all",
            );
    ...
    const [domesticShipping, setDomesticShipping] = useState(() => {
        if (item && item.shop_shipping_info) {
            const domestic = item.shop_shipping_info.find(
                (s) => s.country !== "all",
            );
    ...
    const handleLists = (val) => {
        ...
        setShopItem({
            ...shopItem,
            type: type === "physical" ? "physical" : "Digital Products",
        });
    };
    ...
    const getSubmitData = () => {
        ...
        return {
            ...
            ...rewardPayload,
            reward_file:
                isDigital && reward.type === "file"
                    ? reward.file?.uuid || null
                    : null,
            reward_file_type:
                isDigital && reward.type === "file"
                    ? reward.file?.mime || null
                    : null,
        ...
        axios
            ...
            .then((res) => {
                if (res.data.status) {
                    ...
                    // Reset form after a short delay to let the modal close animation complete
                    setTimeout(() => {
                        resetFormFields();
                        update && update();
    ...
    const handleModalClose = () => {
        // If loading or submitting, don't allow closing
        ...
        if (isDirty()) {
            if (
                !window.confirm(
                    "Discard this product? Your changes haven't been saved.",
                )
            ) {
                return;
            }
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="space-y-1.5">
                                                ...
                                                <div className="relative">
                                                    <div className="absolute left-4 top-1/2 -translate-y-1/2 font-black text-black/60">
                                                        {defaultCurrency}
                                                    </div>
                                                    <input
                                                        name="price"
