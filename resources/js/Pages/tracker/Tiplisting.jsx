   ...
   const TipItem = ({g}) => {
      ...
      return <>
         <div className='box shadow-pink rounded-lg p-3 mb-4 mt-4' >
            <div onClick={openState}  aria-controls="example-collapse-text "  
            aria-expanded={open} className="cursor-pointer trackbar " >
                  <div className='d-flex tip align-items-center justify-content-between' >
                     ...
                     <h2 className='text-large mb-1' >{g && g.tip_goal?.name}</h2>
                        { g && !g.sender 
                           ? <p className=' mb-0' >From : 
                           {g.anonymous == 1 && g.sender == false ? 'Anonymous' : g?.guest_name}</p> 
                     ...
                     <div className="angle-icon w-auto d-flex justify-content-end align-items-center">
                        {g && g.sender ?
                           <div className="identity text-danger text-nowrap" >-{formatMultiPrice(g.amount * (+g.quantity || 1), g?.tip_goal?.currency || 'gbp')}</div>
                           :
                           <div className="identity text-success text-nowrap" >+{formatMultiPrice(g.amount * (+g.quantity || 1), g?.tip_goal?.currency || 'gbp')}</div>
                        }
