    ...
    return (
        <div className={`fading border-2 cartlist flex justify-between content-between items-center border-black/15 rounded-box-sm
                ...
                <div>
                    ...
                    <div className='font-bold'>
                        <div className="flex flex-col">
                            {/* The quantity stepper is hidden, so a row priced
                                per unit next to a basket total for two of them
                                reads as an arithmetic error. */}
                            <span>
                                {formatMultiPrice(totalPrice || ((data.price || 0) + (data.tax || 0)), currency)}
                                {quantity > 1 ? (
                                    <span className="ml-1.5 text-sm font-normal text-black/60">
                                        × {quantity}
                                    </span>
                                ) : null}
                            </span>
            ...
            <div className=' items-center'>
                ...
                {/* 44px minimum — the icon alone was a ~24px tap target. */}
                <button className='del flex h-11 w-11 items-center justify-center' aria-label="Remove from basket" onClick={()=>removeCart(data && data.uuid)} ><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
