...
export default function GifterItems(props) {
  ...
  const Item = ({key, w}) => { 
    ...
    const Template = () => { 
      ...
      return <div className='pb-3 d-flex align-items-start '>

        <div className={`gift-icon mt-2 me-2 ${s == '0' ? 'mint' : s == '1' ? 'pink' : s == '2' ? 'voilet' : 'grey' }`} dangerouslySetInnerHTML={{ __html: piggy }} />
        {s == '0' ? 
