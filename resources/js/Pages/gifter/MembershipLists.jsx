...
import Membership from './../membership/Membership';
  ...
  export default function MembershipLists({username}) {
    ...
    const ITEM = ({itm}) => { 
      return <>
          <div className='relative group overflow-hidden rounded-[30px] md:rounded-[40px]  bg-[#1A1B23]/40 border border-white/5 transition-all duration-500 hover:border-white/20 hover:bg-[#1A1B23]/60 hover:-translate-y-1' >
            ...
            <div className='p-6 relative' >
              <Link href={`${itm?.owner?.username || ''}`} className="flex items-center gap-3 mb-6 group/author" >
                  <div className="relative">
                      <div className="absolute -inset-1 bg-gradient-to-r from-[#05EFB8] to-[#8C52FF] rounded-[30px] md:rounded-[40px]  blur opacity-0 group-hover/author:opacity-40 transition duration-500"></div>
                      <img className="relative h-12 w-12 rounded-[30px] md:rounded-[40px]  object-cover border border-white/10" src={`${itm?.owner?.avatar || ''}`} />
                  </div>
                <div>
