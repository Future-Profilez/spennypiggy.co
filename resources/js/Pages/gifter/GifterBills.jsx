...
export default function GifterBills({ username }) {
  ...
  const BillItem = ({ b, i }) => {
    ...
    return (
      <div key={`bill-${i}`} className="bg-white rounded-[30px] md:rounded-[40px]   shadow-md p-4 mb-3 border border-2 border-pink-300">
        ...
        {b.bill?.perma_link && (
          <div className="mt-3">
            <img src={b.bill.perma_link} alt={b.bill?.name} className="w-full h-40 object-cover rounded-[30px] md:rounded-[40px]  " />
          </div>
        )}
