...
export default function LeaderboardStars() {
  ...
  // This panel never varied by period — it fetched the same URL every time —
  // so it comes straight from the shared bundle. It also used a RELATIVE url,
  // which broke the moment the board rewrote the page URL to /leaderboard/<period>.
  const { data: section, loading, error, retry: fetchGifts } = useBundleSection('stars');
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-box border-black p-4 flex justify-center items-center" style={{minHeight: '200px'}}>
  ...
  if (error) {
    return (
      <div className="bg-white rounded-box border-black p-4 text-center">
  ...
  return (
   <>
    {lists.length > 0 ?  <div className="bg-white rounded-box border-black p-4">
