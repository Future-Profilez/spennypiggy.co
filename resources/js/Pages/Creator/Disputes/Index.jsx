...
export default function DisputesIndex({ auth, disputes, tickets, queries }) {
                ...
                <div className="containerbox">
                    <div className="flex justify-between items-center mb-8">
                        <div>
                            ...
                            <p className="text-white/60 mt-1">Handle disputes and refund requests in one place.</p>
                        </div> 
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3 mb-8">
                        <button
                            ...
                            className={`px-5 py-2 rounded-box-sm text-sm font-black uppercase tracking-widest border-[3px] border-black transition-all flex items-center gap-2 ${
                                activeTab === 'disputes'
 ...
 ? 'bg-yellow-300 text-black translate-x-[-1px] translate-y-[-1px]'
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-8">
                        <div className="bg-[#1a1a1a] rounded-box  p-6 border border-gray-800 flex items-center justify-between">
                            <div>
                                <p className="text-white/60 text-sm font-bold uppercase tracking-wider mb-1">Total Reserved Amount</p>
                        ...
                        <div className="bg-[#1a1a1a] rounded-box  p-6 border border-gray-800 flex items-center justify-between">
                            <div>
                                <p className="text-white/60 text-sm font-bold uppercase tracking-wider mb-1">Dispute Win Rate</p>
                                <h3 className="text-3xl font-bold text-white">
                                    {(() => {
                    ...
                    <div className="bg-[#1a1a1a] rounded-box p-6 md:p-8 border border-gray-800">
                        {activeTab === 'disputes' && (
                            disputes.data.length === 0 ? (
                                <div className="text-center py-16 flex flex-col items-center justify-center">
                                    ...
                                    <p className="text-white/60 max-w-md mx-auto">Great job! You have no active disputes. Keep providing excellent content to your supporters.</p>
                                </div>
                            ) : (
                                    ...
                                    <table className="w-full whitespace-nowrap">
                                        <thead>
                                            <tr className="text-left border-b border-gray-700">
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider">Date</th>
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider">Supporter</th>
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider">Amount</th>
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider">Reason</th>
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider">Status</th>
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider">Due By</th>
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider text-right">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-gray-800">
                                            {disputes.data.map((dispute) => (
                                                <tr key={dispute.id} className="hover:bg-white/5 transition-colors group">
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                <div className="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center overflow-hidden border border-gray-700">
                                                                    {dispute.payment.supporter.avatar ? (
                                                                        ...
                                                                        <span className="text-[12px] font-bold text-white/60">{dispute.payment.supporter.name[0]}</span>
                                                                    )}
                                                                </div>
                                                                <div>
                                                                    <div className="text-white font-medium text-xs">{dispute.payment.supporter.name}</div>
                                                                    <div className="text-white/60 text-[12px]">@{dispute.payment.supporter.username}</div>
                                                                </div>
                                                            </div>
                                                        ) : (
                                                            <span className="text-white/60 italic text-xs">Unknown / Guest</span>
                                                        )}
                                                    </td>
                                                    ...
                                                    <td className="px-6 py-5 text-sm text-white/60 capitalize">
                                                    ...
                                                    <td className="px-6 py-5 text-right">
                                                        <Link 
                                                            ...
                                                            className="inline-flex items-center gap-1.5 bg-gray-800 hover:bg-[#FF007F] text-white text-[12px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-box-sm transition-all"
                                                        >
                                                            Review & Evidence
                                                        </Link>
                                                    </td>
                        ...
                        {activeTab === 'refunds' && (
                            (tickets?.data || []).length === 0 ? (
                                <div className="text-center py-16 flex flex-col items-center justify-center">
                                    ...
                                    <p className="text-white/60 max-w-md mx-auto">You have no pending refund requests.</p>
                                </div>
                            ) : (
                                <div className="overflow-x-auto">
                                    <table className="w-full whitespace-nowrap">
                                        <thead>
                                            <tr className="text-left border-b border-gray-700">
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider">Date</th>
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider">Supporter</th>
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider">Type</th>
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider">Status</th>
                                                <th className="px-6 py-4 text-xs font-bold text-white/60 uppercase tracking-wider text-right">Action</th>
                                            </tr>
                                        </thead>
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                <div className="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center overflow-hidden border border-gray-700">
                                                                    {t.supporter.avatar_url || t.supporter.avatar ? (
                                                                        ...
                                                                        <span className="text-[12px] font-bold text-white/60">{t.supporter.name?.[0] || 'S'}</span>
                                                                    )}
                                                                </div>
                                                                <div>
                                                                    <div className="text-white font-medium text-xs">{t.supporter.name}</div>
                                                                    <div className="text-white/60 text-[12px]">@{t.supporter.username}</div>
                                                                </div>
                                                            </div>
                                                        ) : (
                                                            <span className="text-white/60 italic text-xs">{t.guest_email || 'Guest'}</span>
                                                        )}
                                                    </td>
                                                    <td className="px-6 py-5 text-sm text-white/60 capitalize">{t.type}</td>
                                                    <td className="px-6 py-5">
                        ...
                        {activeTab === 'queries' && (
                            (queries?.data || []).length === 0 ? (
                                <div className="text-center py-16 flex flex-col items-center justify-center">
                                    ...
                                    <p className="text-white/60 max-w-md mx-auto">You have no pending contact queries.</p>
                                </div>
                            ) : (
                                <div className="overflow-x-auto">
                                    <table className="w-full whitespace-nowrap">
