...
const BTN =
    "inline-flex items-center justify-center gap-2 min-h-[44px] px-4 rounded-box-sm text-[14px] font-semibold transition-colors";
        ...
        <section className="mt-10 first:mt-0">
            <div className="flex items-end justify-between gap-4 mb-4">
                <div>
                    ...
                    {sub ? (
                        <p className="text-[13px] text-black/60 font-medium mt-1 leading-snug">
                            {sub}
                        </p>
                    ) : null}
                </div>
    ...
    return (
        <div className={`${CARD} p-5 flex flex-col justify-between`}>
            <div>
                ...
                <div
                    className={`text-2xl md:text-[28px] ${MONEY} mt-2 ${toneCls}`}
                >
                    {value}
                </div>
            </div>
...
export default function Dashboard({
    ...
    const tab = TABS.some((t) => t.key === active_tab)
        ? active_tab
        : "overview";
    ...
    const chartData =
        analytics?.monthly?.map((item) => ({
            name: new Date(item.month + "-01").toLocaleDateString("en-GB", {
                month: "short",
            }),
    ...
    const reserveTotal = Number(
        reserveDetails?.total_held ?? summary?.held_reserves ?? 0,
    );
    const releasedReserves =
        reserveDetails?.released_breakdown ?? reserve_released_breakdown ?? [];
    const releasedTotal = Number(
        reserveDetails?.total_released ?? reserve_total_released ?? 0,
    );
    ...
    const cycleWindowLabel = (() => {
        if (!payout_cycle?.window_start || !payout_cycle?.window_end)
            return null;
        const fmt = (d) =>
    ...
    // The week still being EARNED, and the Friday it will be paid on. Deliberately a
    // separate line from the window above: showing this week beside the next payout date
    // is what made creators read this week's sales as being in Friday's payment.
    const currentWindowLabel = (() => {
        if (
            !payout_cycle?.current_window_start ||
            !payout_cycle?.current_window_end
        )
            return null;
        const fmt = (d) =>
            new Date(d).toLocaleDateString("en-GB", {
        ...
        return `${fmt(payout_cycle.current_window_start)} to ${fmt(payout_cycle.current_window_end)}`;
    })();

    const currentWindowPaidLabel = payout_cycle?.current_window_paid_at
        ? new Date(payout_cycle.current_window_paid_at).toLocaleDateString(
              "en-GB",
    ...
    const nextPayoutLabel = payout_cycle?.next_payout_at
        ? new Date(payout_cycle.next_payout_at).toLocaleDateString("en-GB", {
              weekday: "long",
              day: "2-digit",
              month: "short",
              year: "numeric",
    ...
    const nextReserveRelease = (() => {
        const held = (
            Array.isArray(reserve_breakdown) ? reserve_breakdown : []
        ).filter(
            (r) =>
                (r?.reserve_status ?? "held") !== "released" &&
                Number(r?.amount ?? 0) > 0,
        );
        if (!held.length) return null;
        return held.reduce((best, r) => {
            const days = Math.max(0, Number(r?.days_remaining ?? 9999));
            return best === null || days < best.days
                ? { days, date: r?.release_date ?? null }
                : best;
        }, null);
    })();
    ...
    const upcomingTx = Array.isArray(upcoming_payout?.transactions)
        ? upcoming_payout.transactions
        : [];
    const upcomingPayable = Number(
        summary?.payout_preview?.net_payout ?? summary?.payoutable_balance ?? 0,
    );
    const upcomingReserveHeld = Number(
        summary?.payout_preview?.reserve_held ??
            upcoming_payout?.total_reserve ??
            0,
    );
    const upcomingCount = Number(
        summary?.payout_preview?.payment_count ?? upcomingTx.length,
    );
    ...
    const releasingThisWeek = (() => {
        ...
        return (Array.isArray(reserve_breakdown) ? reserve_breakdown : [])
            .filter(
                (r) =>
                    (r?.reserve_status ?? "held") !== "released" &&
                    Number(r?.days_remaining ?? 9999) <= horizon,
            )
            .reduce(
                (sum, r) =>
                    sum +
                    Number(r?.amount_converted ?? Number(r?.amount ?? 0) / 100),
                0,
            );
    })();
    ...
    const payoutState = (() => {
        ...
        if (auth?.user?.payout_paused_at) {
            return {
                ...
                note: "Payouts are on hold. Contact support if you were not expecting this.",
            };
        }
        if (Number(pp?.negative_balance_after || 0) > 0) {
            return {
                ...
                note: "A previous refund left a negative balance. It is recovered from new earnings before the next payout.",
            };
        }
        if (pp?.is_below_threshold) {
            return {
                tone: "warn",
                label: "Below minimum",
                note: "Your payable balance is under the minimum payout amount. It carries over and pays once it clears.",
            };
        }
        return {
            tone: "good",
            label: daysToPayout === 0 ? "Sending today" : "On schedule",
            note: "Your account is in good standing and payouts run every Friday.",
        };
    })();
    ...
    const moneyRows = (() => {
        const rows = [
            {
                ...
                hint: "Paid by supporters. Becomes payable once its earning week closes and is held.",
                dot: "bg-blue-500",
            },
            {
                key: "pending",
        ...
        const riskAmount =
            Number(summary?.review_holds || 0) + Number(summary?.disputes || 0);
        if (riskAmount > 0) {
            rows.push({
                key: "risk",
    ...
    const payableLines = (() => {
        ...
        const lines = [
            ...
            {
                label: "Reserve held (returned later)",
                amount: -Number(pp.reserve_held || 0),
            },
            {
                label: "Refunds & disputes",
                amount: -Number(pp.refund_disputes || 0),
            },
            { label: "Under review", amount: -Number(pp.review_holds || 0) },
            {
                label: "Balance recovery",
                amount: -Number(pp.negative_balance_before || 0),
            },
    ...
    const statusBuckets = [
        {
            key: "queued",
            label: "Queued for Friday",
            text: "text-emerald-700",
            dot: "bg-emerald-500",
            note: "In the next payout batch.",
        },
        {
            key: "clearing",
            label: "Clearing",
            text: "text-blue-700",
            dot: "bg-blue-500",
            note: "Paid, clearing until its earning week closes.",
        },
        {
            key: "pending",
            ...
            dot: "bg-yellow-500",
            note: "Waiting on task or shop delivery.",
        },
        {
            key: "review_hold",
            label: "Review hold",
            ...
            dot: "bg-purple-500",
            note: "Held for safety or compliance checks.",
        },
        {
            key: "disputed",
            label: "Disputed",
            ...
            dot: "bg-orange-500",
            note: "Removed from your payout balance.",
        },
        {
            key: "refunded",
            label: "Refunded",
            text: "text-red-700",
            dot: "bg-red-500",
            note: "Returned to the supporter.",
        },
    ];
    ...
    const records = [
        {
            ...
            icon: <FileText size={16} className="text-[#FF007F]" />,
        },
        {
    ...
    return (
        <AuthenticatedLayout auth={auth} user={auth.user}>
            ...
            --------------------------------------------------------------- */}
            <Modal
                show={showReserveDetails}
                onClose={() => setShowReserveDetails(false)}
                maxWidth="2xl"
            >
                <div className="bg-white text-gray-900 p-5 md:p-6">
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <div className="flex flex-wrap items-end gap-x-6 gap-y-2 mt-2">
                                <div>
                                    <div
                                        className={`text-xl md:text-2xl ${MONEY} text-gray-900`}
                                    >
                                        {formatCurrency(
                                            reserveTotal,
                                            displayCurrency,
                                    ...
                                    <div className="text-[12px] font-semibold text-cyan-700 mt-0.5">
                                        Still to be released
                                    </div>
                                </div>
                                <div>
                                    <div
                                        className={`text-lg md:text-xl ${MONEY} text-emerald-600`}
                                    >
                                        {formatCurrency(
                                            releasedTotal,
                                            displayCurrency,
                            ...
                            <div className="text-[13px] text-black/60 font-medium mt-2">
                                {reserve_reason ||
                                    "Reserves currently held on your earnings."}
                            </div>
                    ...
                    <div className="mt-5 max-h-[70dvh] overflow-y-auto pr-1">
                            ...
                            <>
                                ...
                                <div className="md:hidden divide-y divide-gray-100 border border-gray-200 rounded-box overflow-hidden">
                                    {(reserves || []).map((r, idx) => {
                                        ...
                                        const isReleased =
                                            r.reserve_status === "released";
                                        return (
                                            <div
                                                key={`m-${r.financial_transaction_id || idx}-${idx}`}
                                                className="p-4"
                                            >
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <div className="text-[12px] text-black/60 font-medium mt-1">
                                                            {txDate
                                                                ...
                                                                : "-"}
                                                            {r.supporter
                                                                ? ` · @${r.supporter.username}`
                                                                : " · Guest"}
                                                        </div>
                                                    </div>
                                                    <div
                                                        className={`${MONEY} text-gray-900 shrink-0`}
                                                    >
                                                        {formatCurrency(
                                                            Number(
                                                                r.amount || 0,
                                                ...
                                                <div className="mt-2 text-[13px] font-semibold">
                                                    {isReleased ? (
                                                        ...
                                                        <span className="text-cyan-700">
                                                            {relDays === 0
                                                                ? "Frees today"
                                                                : `Frees in ${relDays} day${plural(relDays)}`}
                                                            {r.release_date
                                                                ? ` · ${r.release_date}`
                                                                : ""}
                                                        </span>
                                    ...
                                    <table className="w-full text-left">
                                        ...
                                        <tbody className="divide-y divide-gray-100">
                                            {(reserves || []).map((r, idx) => {
                                                ...
                                                return (
                                                    <tr
                                                        key={`${r.financial_transaction_id || r.payout_run_id || idx}-${idx}`}
                                                        className="hover:bg-gray-50 transition-colors"
                                                    >
                                                        <td className="px-4 py-3 text-[14px] text-gray-900 font-medium whitespace-nowrap">
                                                            {dateLabel}
                                                        </td>
                                                        ...
                                                        <td
                                                            className={`px-4 py-3 text-right whitespace-nowrap ${MONEY} text-gray-900`}
                                                        >
                                                            {formatCurrency(
                                                                Number(
                                                                    r.amount ||
                                                        ...
                                                        <td className="px-4 py-3 text-right whitespace-nowrap">
                                                            {statusMeta ? (
                                                                <span
                                                                    className={`inline-block px-2.5 py-1 rounded-full text-[12px] font-semibold border ${statusMeta.cls}`}
                                                                >
                                                                    {
                                                                        statusMeta.label
                                                                    }
                        ...
                        {!reserveLoading &&
                            !reserveLoadError &&
                            (releasedReserves || []).length > 0 && (
                                <div className="mt-6">
                                    <div className="text-[13px] font-semibold text-emerald-700 mb-2">
                                        Released history — reserves already paid
                                        back to you
                                    </div>
                                    <div className="divide-y divide-gray-100 border border-gray-200 rounded-box overflow-hidden">
                                        {(releasedReserves || []).map(
                                            (r, idx) => {
                                                const relAt = r.released_at
                                                    ? new Date(r.released_at)
                                                    : null;
                                                const verified =
                                                    r.verified !== false;
                                                return (
                                                    <div
                                                        key={`rel-${r.financial_transaction_id || idx}-${idx}`}
                                                        className="flex items-start justify-between gap-3 p-4"
                                                    >
                                                        <div className="min-w-0">
                                                            <div className="text-[14px] font-medium text-gray-900 line-clamp-2">
                                                                {r.source_name ||
                                                                    "Reserve"}
                                                            </div>
                                                            <div className="text-[12px] text-black/60 font-medium mt-1">
                                                                {relAt
                                                                    ? relAt.toLocaleDateString(
                                                                          "en-GB",
                                                                          {
                                                                              day: "numeric",
                                                                              month: "short",
                                                            ...
                                                            <div
                                                                className={`text-[12px] font-semibold mt-1 ${verified ? "text-emerald-600" : "text-amber-600"}`}
                                                            >
                                                                {verified
                                                                    ? "Paid to you"
                                                                    : "Settled with base earning"}
                                                            </div>
                                                        </div>
                                                        <div
                                                            className={`${MONEY} text-gray-900 shrink-0`}
            ...
            <div className="bg-gray-50 min-h-dvh pb-28 md:pb-12">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                        <div className="min-w-0">
                            ...
                            <p className="text-[14px] md:text-[15px] text-gray-600 font-medium mt-1.5 leading-snug">
                                You keep 100% of your listed price — supporters
                                cover the fees. Payouts go out every Friday.
                            </p>
                        ...
                        <div className="grid grid-cols-2 lg:flex gap-2 shrink-0">
                            <Link
                                href={route("financial.expenses.index")}
                                onMouseEnter={() =>
                                    logExpenseIconRef.current?.startAnimation?.()
                                }
                                onMouseLeave={() =>
                                    logExpenseIconRef.current?.stopAnimation?.()
                                }
                            ...
                            <a
                                ...
                                rel="noreferrer"
                                onMouseEnter={() =>
                                    exportCsvIconRef.current?.startAnimation?.()
                                }
                                onMouseLeave={() =>
                                    exportCsvIconRef.current?.stopAnimation?.()
                                }
                                className={`${BTN} bg-white border border-gray-200 hover:bg-gray-100 text-gray-800`}
                            >
                                <DownloadIcon
                                    ref={exportCsvIconRef}
                                    size={18}
                                />
                                ...
                                <div>
                                    ...
                                    <div className="text-[14px] text-red-800 font-medium mt-1">
                                        {auth.user.payout_pause_reason ||
                                            "Under review by SpennyPiggy support."}
                                    </div>
                                    <div className="text-[13px] text-red-700 font-medium mt-1">
                                        ...
                                        · your earnings are safe and will be
                                        sent once this clears.
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="mt-6 rounded-box border border-emerald-100 bg-white overflow-hidden">
                            ...
                            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-5">
                                <div className="min-w-0">
                                    ...
                                    <div className="mt-3 flex items-end gap-3 flex-wrap">
                                        ...
                                        <span
                                            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[12px] font-semibold mb-1 ${stateChip}`}
                                        >
                                            <span
                                    ...
                                    <div className="mt-2.5 text-[14px] text-gray-600 font-medium leading-snug">
                                        ...
                                        {upcomingCount > 0
                                            ...
                                            : "Nothing eligible yet — a sale is paid once its Friday-to-Thursday week closes and is held for a week."}
                                    </div>
                                    {payoutState.tone !== "good" ? (
                                        <div className="mt-2 text-[13px] text-gray-600 font-medium leading-snug max-w-xl">
                                            {payoutState.note}
                                        </div>
                                    ) : null}
                                </div>
                                ...
                                <div className="shrink-0 flex items-center gap-3">
                                    <Link
                                        ...
                                        className="inline-flex items-center gap-1 text-[14px] font-semibold text-[#FF007F] hover:underline min-h-[44px]"
                                    >
                                        Payout history{" "}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 border-t border-gray-100 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
                            <div className="p-5">
                                ...
                                <div
                                    className={`text-xl ${MONEY} text-gray-900 mt-2`}
                                >
                                    {formatCurrency(
                                        summary?.clearing_balance,
                                        displayCurrency,
                                    )}
                                </div>
                                <div className="text-[12px] text-black/60 font-medium mt-1">
                                    Payable once its earning week closes
                                </div>
                            </div>
                            <button
                                type="button"
                                onClick={openReserveDetails}
                                className="p-5 text-left hover:bg-gray-50 transition-colors min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-400/50"
                            >
                                <div
                                    className={`${LABEL} flex items-center gap-2`}
                                >
                                ...
                                >
                                    {formatCurrency(
                                        summary?.held_reserves,
                                        displayCurrency,
                                    )}
                                </div>
                                <div className="text-[12px] font-semibold text-[#FF007F] mt-1">
                                    {releasingThisWeek > 0
                                        ? `${formatCurrency(releasingThisWeek, displayCurrency)} frees this week →`
                            ...
                            <div className="p-5">
                                ...
                                <div className="text-[12px] text-black/60 font-medium mt-1">
                                    Yours once you fulfil the order
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <Link
                                    ...
                                    className={`px-4 min-h-[48px] flex items-center font-semibold text-[14px] border-b-2 whitespace-nowrap transition-colors ${
                                        tab === t.key
                                            ...
                                            : "border-transparent text-black/60 hover:text-gray-900"
                                    }`}
                                >
                                    {t.label}
                                </Link>
                    ...
                    {tab === "overview" && (
                        <div className="pt-8">
                            <Section
                                ...
                                sub="Every pound you've earned, and which stage it's at right now."
                            >
                                <div className={`${CARD} overflow-hidden`}>
                                    <div className="divide-y divide-gray-100">
                                        {moneyRows.map((row) => {
                                            const Cmp = row.onClick
                                                ? "button"
                                                : "div";
                                            return (
                                                <Cmp
                                                    ...
                                                    className={`w-full text-left flex items-center justify-between gap-4 px-5 py-4 min-h-[44px] ${
                                                        row.onClick
                                                            ? "hover:bg-gray-50 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#FF007F]/40"
                                                            : ""
                                                    } ${row.muted ? "bg-emerald-50/40" : ""}`}
                                                >
                                                    <div className="flex items-start gap-3 min-w-0">
                                                        <span
                                                            className={`mt-1.5 h-2.5 w-2.5 rounded-full shrink-0 ${row.dot}`}
                                                        />
                                                        <div className="min-w-0">
                                                            <div className="text-[15px] font-semibold text-gray-900 leading-tight">
                                                                {row.label}
                                                                {row.onClick ? (
                                                                    <span className="text-[13px] text-[#FF007F] font-semibold ml-2">
                                                                        View →
                                                                    </span>
                                                                ) : null}
                                                            </div>
                                                    ...
                                                    <div
                                                        className={`text-lg ${MONEY} whitespace-nowrap ${row.muted ? "text-emerald-600" : "text-gray-900"}`}
                                                    >
                                                        {formatCurrency(
                                                            row.amount,
                                                            displayCurrency,
                                    ...
                                    <div className="px-5 py-3.5 bg-gray-50 border-t border-gray-100 text-[13px] text-gray-600 font-medium flex items-center gap-2">
                                        ...
                                        />
                                        Reserves are returned automatically 30
                                        ...
                                        >
                                            <span className="text-[14px] font-semibold text-gray-800">
                                                {showPayoutBreakdown
                                                    ? "Hide"
                                                    : "Show"}{" "}
                                                the {upcomingTx.length} sale
                                                {plural(upcomingTx.length)} in
                                                this payout
                                            </span>
                                            <ChevronRightIcon
                                                size={18}
                                                className={`text-black/60 transition-transform duration-200 ${showPayoutBreakdown ? "rotate-90" : ""}`}
                                            />
                                        </button>
                                        {showPayoutBreakdown && (
                                            <div
                                                            ...
                                                            return (
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="text-right shrink-0">
                                                                        <div
                                                                            className={`${MONEY} text-emerald-600`}
                                                                        >
                                                                            {formatCurrency(
                                                                                t.payable_converted,
                                                                                displayCurrency,
                                                                        ...
                                                                        0 ? (
                                                                            <div className="text-[12px] text-cyan-700 font-semibold mt-0.5 tabular-nums">
                                                                                {formatCurrency(
                                                                                    t.reserve_converted,
                                                                                    displayCurrency,
                                                                                )}{" "}
                            ...
                            <Section
                                title={`Earnings this tax year`}
                                sub={`Totals for ${tax_year}. Full breakdown lives in the Tax tab.`}
                            >
                            ...
                            <Section
                                ...
                                sub={`Month by month across ${tax_year}.`}
                            >
                            ...
                            <Section
                                title="What's earning"
                                sub="Which products bring the money in, and who your best supporters are."
                            >
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                                ...
                                                return (
                                                    <div key={key}>
                                                        <div className="flex items-center justify-between gap-3 mb-1.5">
                                                            ...
                                                            <span
                                                                className={`text-[14px] ${MONEY} text-gray-900`}
                                                            >
                                                                {formatCurrency(
                                                                    t?.total ??
                                                                        0,
                                    ...
                                    >
                                        ...
                                        {(top_supporters || []).length === 0 ? (
                                            <div className="px-5 pb-6 text-[14px] text-black/60 font-medium">
                                                No supporters yet this tax year.
                                            </div>
                                        ) : (
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3 min-w-0">
                                                            ...
                                                            <div className="min-w-0">
                                                                ...
                                                                <div className="text-[12px] text-black/60 font-medium truncate">
                                                                    @
                                                                    {s.supporter
                                                                        ?.username ??
                                                                        "guest"}
                                                        ...
                                                        <div
                                                            className={`text-[15px] ${MONEY} text-gray-900 shrink-0`}
                                                        >
                                                            {formatCurrency(
                                                                s.total_spent,
                                                                displayCurrency,
                                        ...
                                        >
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <div className="text-[12px] text-black/60 font-medium">
                                                    Who's gone quiet, and what
                                                    to do next.
                                                </div>
                                            </div>
                                            <ChevronRightIcon
                                                size={18}
                                                className="text-[#FF007F] shrink-0"
                                            />
                                        </Link>
                                    </div>
                                </div>
                            ...
                            <Section
                                ...
                                sub={`Every payment recorded in ${tax_year}.`}
                            >
                                <LedgerHistoryTable
                    ...
                    {tab === "payouts" && (
                        <div className="pt-8">
                            <Section
                                title="How this payout is worked out"
                                sub="From what you earned, to what lands in your bank."
                            >
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                    <div className={`${CARD} p-5 md:p-6`}>
                                        <div className={LABEL}>
                                        ...
                                        <div
                                            className={`text-3xl ${MONEY} text-emerald-600 mt-2`}
                                        >
                                            {formatCurrency(
                                                summary.payoutable_balance,
                                                displayCurrency,
                                            )}
                                        </div>
                                        {payableLines.length > 0 ? (
                                            <div className="mt-4 space-y-2 border-t border-gray-100 pt-3">
                                                {payableLines.map(
                                                    (line, idx) => (
                                                        <div
                                                            key={idx}
                                                            className="flex justify-between items-center gap-3 text-[13px] font-medium"
                                                        >
                                                            <span className="text-black/60">
                                                                {line.label}
                                                            </span>
                                                            <span
                                                                className={`tabular-nums font-semibold whitespace-nowrap ${line.amount >= 0 ? "text-gray-800" : "text-red-600"}`}
                                                            >
                                                                {line.amount >=
                                                                0
                                                                    ? "+"
                                                ...
                                                <div className="flex justify-between items-center gap-3 text-[14px] font-bold border-t border-gray-100 pt-2.5">
                                                    ...
                                                    <span className="tabular-nums text-emerald-600 whitespace-nowrap">
                                                        {formatCurrency(
                                                            summary.payoutable_balance,
                                                            displayCurrency,
                                                        )}
                                            ...
                                            <div className="text-[13px] text-black/60 font-medium mt-3 leading-snug">
                                                Excludes reserves, orders
                                                awaiting delivery, disputes and
                                                refunds.
                                            </div>
                                        )}
                                        ...
                                        {summary.carry_over_amount > 0 ? (
                                            <div className="text-[13px] text-black/60 font-medium mt-3">
                                                Includes{" "}
                                                {formatCurrency(
                                                    summary.carry_over_amount,
                                                    displayCurrency,
                                                )}{" "}
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        <div className={`${CARD} p-5 md:p-6`}>
                                            ...
                                            <div className="text-[16px] font-semibold text-gray-900 mt-2">
                                                {nextPayoutLabel ||
                                                    "Every Friday"}
                                            </div>
                                        ...
                                        >
                                            ...
                                            <div
                                                className={`text-[22px] ${MONEY} text-gray-900 mt-2`}
                                            >
                                                {formatCurrency(
                                                    summary?.held_reserves ?? 0,
                                                    displayCurrency,
                                                )}
                                            </div>
                                            <div className="text-[13px] text-black/60 font-medium mt-1.5 leading-snug">
                                                {(reserve_policy?.effective_percent ??
                                                    0) > 0
                                            ...
                                            {reserve_policy?.onboarding_percent >
                                                0 &&
                                            reserve_policy?.onboarding_ends_at ? (
                                                <div className="text-[13px] text-black/60 font-medium mt-1">
                                                    New-creator hold ends{" "}
                                                    {
                                                        reserve_policy.onboarding_ends_at
                                                    }
                                                    .
                                            ...
                                            <div className="text-[13px] font-semibold text-[#FF007F] mt-2">
                                                See every held reserve →
                                            </div>
                                        </button>
                                    </div>
                                </div>
                            ...
                            <Section
                                title="Every pound, by status"
                                sub="Nothing is lost — each bucket says exactly why it isn't payable yet."
                            >
                                <div
                                        ...
                                        ({ key, label, dot, note }) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className="flex items-start gap-3 min-w-0">
                                                        <span
                                                            className={`mt-1.5 h-2.5 w-2.5 rounded-full shrink-0 ${dot}`}
                                                        />
                                                        <div className="min-w-0">
                                                            <div className="text-[15px] font-semibold text-gray-900 leading-tight">
                                                                {label}
                                                    ...
                                                    <div
                                                        className={`text-lg ${MONEY} text-gray-900 whitespace-nowrap`}
                                                    >
                                                        {formatCurrency(
                                                            total,
                                                            displayCurrency,
                            ...
                            */}
                            ...
                            {(fast_start_bonus || founder_bonus) && (
                                <Section
                                    title="Your bonuses"
                                    sub="Platform rewards paid on top of your earnings."
                                >
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                        {fast_start_bonus
                                            ? (() => {
                                                  const start =
                                                      fast_start_bonus.window_start
                                                          ? new Date(
                                                                fast_start_bonus.window_start,
                                                            )
                                                          : null;
                                                  const end =
                                                      fast_start_bonus.window_end
                                                          ? new Date(
                                                                fast_start_bonus.window_end,
                                                            )
                                                          : null;
                                                  ...
                                                  return (
                                                      ...
                                                      >
                                                          <div className="flex items-center gap-2 flex-wrap">
                                                              ...
                                                              <span
                                                                  className={`px-2.5 py-1 rounded-full text-[12px] font-semibold border ${isActive ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-gray-100 text-gray-600 border-gray-200"}`}
                                                              >
                                                                  {String(
                                                                      fast_start_bonus.status ||
                                                                          "",
                                                          ...
                                                          <div
                                                              className={`text-3xl ${MONEY} text-gray-900 mt-3`}
                                                          >
                                                              {formatCurrency(
                                                                  fast_start_bonus.bonus_so_far ||
                                                                      0,
                                                                  fast_start_bonus.currency,
                                                              )}
                                                          </div>
                                                          <div className="text-[14px] text-gray-600 font-medium mt-1.5">
                                                              On{" "}
                                                              {formatCurrency(
                                                                  fast_start_bonus.earnings_so_far ||
                                                                      0,
                                                                  fast_start_bonus.currency,
                                                              )}{" "}
                                                              of tracked
                                                          ...
                                                          {isActive ? (
                                                              <div className="mt-4">
                                                                  <div className="flex justify-between text-[13px] text-black/60 font-medium mb-1.5">
                                                                      <span>
                                                                          {fast_start_bonus.days_remaining ??
                                                                              "-"}{" "}
                                                                          day
                                                                          {plural(
                                                                              fast_start_bonus.days_remaining,
                                                          ...
                                                          <Link
                                                              href={route(
                                                                  "financial.fast-start-bonus",
                                                              )}
                                                              className="inline-flex items-center gap-1 text-[13px] font-semibold text-[#FF007F] hover:underline mt-3 min-h-[44px]"
                                                          >
                                                              Full details{" "}
                                                              <ChevronRightIcon
                                                                  size={15}
                                        ...
                                        {founder_bonus
                                            ? (() => {
                                                  const start =
                                                      founder_bonus.month_start
                                                          ? new Date(
                                                                founder_bonus.month_start,
                                                            )
                                                          : null;
                                                  const end =
                                                      founder_bonus.month_end
                                                          ? new Date(
                                                                founder_bonus.month_end,
                                                            )
                                                          : null;
                                                  ...
                                                  const progress =
                                                      start && end
                                                          ...
                                                          : 0;
                                                  const qualification =
                                                      founder_bonus.qualification_payout ||
                                                      null;
                                                  const lastMonth =
                                                      founder_bonus.last_month ||
                                                      null;
                                                  const isActive =
                                                      founder_bonus.status ===
                                                      "active";
                                                  return (
                                                      <div
                                                          className={`${CARD} p-5 md:p-6`}
                                                      >
                                                          ...
                                                          >
                                                              {formatCurrency(
                                                                  founder_bonus.bonus_so_far ||
                                                                      0,
                                                                  founder_bonus.currency,
                                                              )}
                                                          </div>
                                                          <div className="text-[14px] text-gray-600 font-medium mt-1.5">
                                                              On{" "}
                                                              {formatCurrency(
                                                                  founder_bonus.earnings_so_far ||
                                                                      0,
                                                                  founder_bonus.currency,
                                                              )}{" "}
                                                              earned this month
                                                          ...
                                                          <div className="mt-4">
                                                              <div className="flex justify-between text-[13px] text-black/60 font-medium mb-1.5">
                                                                  ...
                                                                  <span>
                                                                      {founder_bonus.months_left ===
                                                                          null ||
                                                                      founder_bonus.months_left ===
                                                                          undefined
                                                                          ? "-"
                                                                          : `${founder_bonus.months_left} month${plural(founder_bonus.months_left)} left`}
                                                                  </span>
                                                              </div>
                                                              <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                                                                  <div
                                                          ...
                                                          <div className="text-[13px] text-black/60 font-medium mt-3 leading-snug">
                                                              {Math.round(
                                                                  (founder_bonus.bonus_percentage ||
                                                                      0) * 100,
                                                              )}
                                                              % bonus · min{" "}
                                                              {formatCurrency(
                                                                  founder_bonus.min_monthly_earnings ||
                                                                      0,
                                                                  founder_bonus.currency,
                                                              )}{" "}
                                                              · cap{" "}
                                                              {formatCurrency(
                                                                  founder_bonus.max_bonus_per_month ||
                                                                      0,
                                                                  founder_bonus.currency,
                                                              )}
                                                          </div>
                                                              ...
                                                              <div className="mt-4 rounded-box-sm bg-gray-50 border border-gray-200 px-4 py-3 flex items-center justify-between gap-3">
                                                                  <div className="min-w-0">
                                                                      ...
                                                                      <div className="text-[12px] text-black/60 font-medium mt-0.5">
                                                                          ...
                                                                          {qualification.estimated_payout_date
                                                                              ? ` · due ${new Date(qualification.estimated_payout_date).toLocaleDateString("en-GB")}`
                                                                              : ""}
                                                                      </div>
                                                                  </div>
                                                                  <div
                                                          ...
                                                          {lastMonth ? (
                                                              <div className="mt-3 rounded-box-sm bg-gray-50 border border-gray-200 px-4 py-3 flex items-center justify-between gap-3">
                                                                  <div className="min-w-0">
                                                                      ...
                                                                      <div className="text-[12px] text-black/60 font-medium mt-0.5">
                                                                          {String(
                                                                              lastMonth.payout_status ||
                                                                                  "",
                                                                          ).replaceAll(
                                                                              "_",
                                                                              " ",
                                                                          )}
                                                                          {lastMonth.payout_date
                                                                              ? ` · ${new Date(lastMonth.payout_date).toLocaleDateString("en-GB")}`
                                                                              : ""}
                                                                      </div>
                                                                  </div>
                                                                  <div className="text-right shrink-0">
                                                                      <div
                                                                          className={`${MONEY} text-gray-900`}
                                                                      >
                                                                          {formatCurrency(
                                                                              lastMonth.bonus_amount ||
                                                                                  0,
                                                                              founder_bonus.currency,
                                                                          )}
                                                                      </div>
                            ...
                            <Section
                                ...
                                action={
                                    <span className="text-[13px] text-black/60 font-medium tabular-nums">
                                        {payout_history.length} payout
                                        {plural(payout_history.length)}
                                    </span>
                                }
                            >
                                <div className={`${CARD} overflow-hidden`}>
                                    {payout_history.length === 0 ? (
                                        <div className="px-6 py-14 text-center flex flex-col items-center gap-2">
                                            ...
                                            <span className="text-black/60 text-[13px] font-medium">
                                                Your first payout lands 8 to 14
                                                days after your first sale,
                                                depending which day of the week
                                                it falls on. After that you are
                                                paid every Friday.
                                            </span>
                                        </div>
                                    ) : (
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div className="flex items-start justify-between gap-3">
                                                                <div>
                                                                    <div
                                                                        className={`text-xl ${MONEY} text-gray-900`}
                                                                    >
                                                                        {formatCurrency(
                                                                            p.amount,
                                                                            p.currency,
                                                                    ...
                                                                    <div className="text-[13px] text-black/60 font-medium mt-0.5">
                                                                        {p.date}
                                                                        {p.time
                                                                            ? ` · ${p.time}`
                                                                            : ""}
                                                                ...
                                                                <span
                                                                    className={`whitespace-nowrap inline-flex items-center px-2.5 py-1 rounded-full text-[12px] font-semibold ${payoutStatusBadgeCls(p.status)}`}
                                                                >
                                                                    {p.status ===
                                                                    "in_transit"
                                                                        ? "On its way"
                                                            ...
                                                            <div className="mt-3 flex items-center gap-2 flex-wrap">
                                                                <span
                                                                    className={`inline-flex items-center px-2.5 py-1 rounded-full text-[12px] font-semibold border ${PAYOUT_TYPE_STYLES[p.type_key] || PAYOUT_TYPE_STYLES.weekly}`}
                                                                >
                                                                    {
                                                                        p.type_label
                                                                    }
                                                                ...
                                                                {hasBonus ? (
                                                                    <span className="text-[12px] font-semibold text-[#FF007F] tabular-nums">
                                                                        Bonus{" "}
                                                                        {formatCurrency(
                                                                            bonusAmt,
                                                                            p.currency,
                                                            ...
                                                            <div className="mt-2 text-[13px] font-medium">
                                                                {isFail ? (
                                                                    ...
                                                                    >
                                                                        {p.failure_reason ||
                                                                            "Payout failed"}
                                                                        <span className="block text-[12px] text-black/60 font-medium mt-0.5">
                                                                            {isOpen
                                                                                ? "Hide details"
                                                                                : "Show details"}
                                                                        </span>
                                                                ...
                                                                isOpen && (
                                                                    <div className="mt-3 rounded-box-sm border border-red-200 bg-red-50 p-4">
                                                                        <p className="text-[13px] text-gray-700 break-words">
                                                                            {p.failure_detail ||
                                                                                "No additional detail provided by Stripe."}
                                                                        </p>
                                                                        {p.failure_code ? (
                                                                            <div className="text-[12px] text-black/60 font-mono mt-2">
                                                                                Code:{" "}
                                                                                {
                                                                                    p.failure_code
                                                                                }
                                                                        ...
                                                                        <p className="text-[13px] text-gray-600 mt-3">
                                                                            ...
                                                                            <a
                                                                                ...
                                                                                className="text-[#FF007F] font-semibold underline"
                                                                            >
                                                                    ...
                                                                    >
                                                                        <tr className="hover:bg-gray-50 transition-colors align-top">
                                                                            <td className="px-6 py-4 whitespace-nowrap">
                                                                                <div className="text-[14px] text-gray-900 font-semibold leading-tight">
                                                                                    {
                                                                                        p.date
                                                                                    }
                                                                                </div>
                                                                            ...
                                                                            <td className="px-6 py-4">
                                                                                <span
                                                                                    className={`whitespace-nowrap inline-flex items-center px-2.5 py-1 rounded-full text-[12px] font-semibold border ${PAYOUT_TYPE_STYLES[p.type_key] || PAYOUT_TYPE_STYLES.weekly}`}
                                                                                >
                                                                                    {
                                                                                        p.type_label
                                                                                    }
                                                                            ...
                                                                            <td className="px-6 py-4 text-right">
                                                                                <div
                                                                                    className={`text-[15px] ${MONEY} text-gray-900`}
                                                                                >
                                                                                    {formatCurrency(
                                                                                        p.amount,
                                                                                        p.currency,
                                                                                ...
                                                                                {hasBonus ? (
                                                                                    <div className="text-[12px] font-semibold tabular-nums text-[#FF007F] mt-0.5">
                                                                                        Bonus{" "}
                                                                                        {formatCurrency(
                                                                                            bonusAmt,
                                                                                            p.currency,
                                                                                ...
                                                                                {p.reference ? (
                                                                                    <div
                                                                                        className="text-[12px] text-black/60 font-mono mt-1 truncate max-w-[140px] ml-auto"
                                                                                        title={
                                                                                            p.reference
                                                                                        }
                                                                                    >
                            ...
                            <Section
                                title="Full ledger"
                                sub="Every transaction, all time."
                            >
                                <LedgerHistoryTable
                                    transactions={recent_transactions}
                                    tax_year={tax_year}
                    ...
                    {tab === "tax" && (
                        <div className="pt-8">
                            <Section
                                title={`Tax year ${tax_year}`}
                                sub={`${date_range?.start} to ${date_range?.end}${tax_band_label ? ` · ${tax_band_label} bands` : ""}. Estimates only — not tax advice.`}
                            >
                                ...
                                <div className={`${CARD} mt-4 p-5 md:p-6`}>
                                    <div className="flex items-start justify-between gap-4 flex-wrap">
                                        <div>
                                            ...
                                            <div className="text-[15px] font-semibold text-gray-900 mt-1.5">
                                                {Math.round(taxYearProgress)}%
                                                through {tax_year}
                                            </div>
                                        </div>
                                        <div className="text-[13px] text-black/60 font-medium">
                                            6 April → 5 April
                                        </div>
                                    </div>
                            ...
                            <Section
                                ...
                                sub="Everything you or your accountant might need to download."
                            >
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                    <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div className="sm:col-span-2">
                                            <StatementDownloadCard
                                                taxYear={tax_year_number}
                                            />
                                        </div>
                                        {records.map((item, idx) => (
                                            ...
                                            >
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    <div className="text-[13px] text-black/60 font-medium mt-0.5 leading-snug">
                                                        {item.sub}
                                                    </div>
                                                </div>
                                                <ChevronRightIcon
                                                    size={18}
                                                    className="text-black/60 shrink-0 mt-1"
                                                />
                                            </Link>
                                        ))}
                                    </div>
                                        ...
                                        {isEditingProfile ? (
                                            ...
                                            >
                                                ...
                                                <div className="flex gap-2 justify-end pt-1">
                                                    ...
                                                    <button
                                                        type="submit"
                                                        disabled={processing}
                                                        className={`${BTN} bg-[#FF007F] hover:brightness-110 disabled:opacity-60 text-black`}
                                                    >
                                                        {processing
                                                            ? "Saving…"
                                                            : "Save"}
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-[15px] font-semibold text-gray-900 mt-1 tabular-nums">
                                                        {profile?.tax_percentage ??
                                                            20}
                                                        %
                                                    </div>
                                                </div>
                                                <Link
                                                    ...
                                                    className={`${BTN} w-full bg-gray-900 hover:bg-black text-white`}
                                                >
                            ...
                            <Section
                                title="Transactions this tax year"
                                sub={`Everything counted towards your ${tax_year} figures.`}
                            >
                                <LedgerHistoryTable
                                    transactions={recent_transactions}
                                    tax_year={tax_year}
