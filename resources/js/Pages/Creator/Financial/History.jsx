...
export default function History({ auth, transactions }) {
        ...
        <AuthenticatedLayout auth={auth} user={auth.user}>
            ...
            <div className="bg-[#A2E4B8] min-h-dvh pb-12">
                <div className="py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-6">
                    {/* Header Section */}
 <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 border-[3px] border-black rounded-box relative overflow-hidden">
 ...
 className="p-3 rounded-box-sm bg-white border-[2px] border-black hover:bg-yellow-300 text-black hover:translate-x-[-1px] hover:translate-y-[-1px] transition-all"
                        ...
                        <a
                            ...
                            target="_blank"
 className="flex items-center gap-2 bg-[#FF90E8] hover:bg-[#ff7ae4] text-black px-6 py-3 rounded-box-sm border-[3px] border-black font-black uppercase tracking-widest text-xs transition-all hover:translate-x-[-1px] hover:translate-y-[-1px] active:translate-x-[1px] active:translate-y-[1px] "
                    ...
                    {/* Table Container */}
 <div className="bg-white border-[1px] border-black rounded-box !overflow-hidden ">
 <div className="overflow-x-auto ">
 <table className=" rounded-box w-full text-left border-collapse">
 <thead className="bg-[#FFE951] border-b-[3px] !border-r-[2px] !border-l-0 !border-t-0 border-black">
 <tr className="text-black text-[12px] !border-l-0 !border-t-0 md:text-xs uppercase font-black tracking-widest">
 ...
 <FileTextIcon size={24} className="text-black/60" />
                                                    </div>
...
const TransactionRow = ({ tx, formatCurrency }) => {
 ...
 <span className="text-[12px] text-black/60 font-black">{new Date(tx.display_date).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
            </td>
 ...
 <span className="text-[12px] font-bold text-black/60 uppercase mt-0.5 tracking-wider">@{tx.supporter.username}</span>
                    </div>
                ) : (
 ...
 <div className="inline-block mt-1.5 px-1.5 py-0.5 bg-gray-100 border-[1.5px] border-black rounded text-[12px] text-gray-600 font-black uppercase tracking-widest">
                    {tx.label || tx.source_type?.split('\\').pop().replace('Payment', '').replace('Purchase', '') || 'Manual'}
                </div>
                {tx.payout_badge === 'this_week' && (
 <div className="inline-block mt-1.5 ml-1.5 px-2 py-0.5 bg-[#FFE951] border-[1.5px] border-black rounded text-[12px] text-black font-black uppercase tracking-widest ">
                        In this week's payout
                    </div>
                )}
                {tx.payout_badge === 'paid_out' && (
 <div className="inline-block mt-1.5 ml-1.5 px-2 py-0.5 bg-[#90FFB1] border-[1.5px] border-black rounded text-[12px] text-black font-black uppercase tracking-widest ">
                        Paid out
                    </div>
                )}
 ...
 <span className={`px-2 py-0.5 rounded-full text-[12px] font-black uppercase border-[1.5px] border-black ${
                            tx.reserve_status === 'released' ? 'bg-[#90FFB1]' : 'bg-[#90E0FF]'
                        }`}>
                            {tx.reserve_status === 'released' ? 'Settled' : 'Held'}
                        </span>
 <span className="text-[12px] font-black text-gray-800">{tx.reserve_percent}%</span>
 ...
 <span className="text-[12px] text-black/60 font-bold uppercase tracking-wide">of your amount</span>
                    </div>
                ) : (
 ...
 <div className="text-[12px] text-black/60 font-black uppercase italic mt-1">
            ...
            <td className="px-6 py-4 text-xs text-center">
                <div className="flex flex-col items-center gap-2">
                    {tx.status && (
 <span className={`inline-block px-3 py-1 rounded-box-sm text-[12px] font-black uppercase tracking-[0.1em] border-[1.5px] border-black ${
 ...
 className={`inline-block px-2 py-0.5 rounded text-[12px] font-black uppercase tracking-[0.1em] border-[1px] border-black ${
                    ...
                    {tx.item_status && (
 <span className={`inline-block px-2 py-0.5 rounded text-[12px] font-black uppercase tracking-[0.1em] border-[1px] border-black ${
 ...
 className="px-2 py-0.5 bg-white border-[1.5px] border-black rounded-box-sm text-[12px] font-black uppercase tracking-widest text-black hover:bg-black hover:text-white transition-all hover:translate-x-[-0.5px] hover:translate-y-[-0.5px] flex items-center gap-1 group/evidence"
...
const PaginationLink = ({ link, index }) => {
    ...
    return (
        <Link
            href={link.url || '#'}
 className={`px-4 py-2 rounded-box-sm text-[12px] font-black border-[2px] border-black transition-all flex items-center gap-1 ${
                link.active
 ? 'bg-[#FF90E8] text-black translate-x-[-1px] translate-y-[-1px] ' 
                    : link.url
 ? 'bg-white text-black hover:bg-[#FFE951] hover:translate-x-[-1px] hover:translate-y-[-1px] ' 
 : 'bg-gray-200 text-black/60 cursor-not-allowed border-gray-300 '
            }`}
            onMouseEnter={() => iconRef.current?.startAnimation?.()}
        >
