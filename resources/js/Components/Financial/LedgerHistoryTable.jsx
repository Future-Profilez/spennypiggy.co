...
export default function LedgerHistoryTable({
    ...
    const isPending = (tx) =>
        ...
        (tx.counts_toward_totals === undefined &&
            (tx.status !== "completed" ||
                (tx.item_status && tx.item_status.endsWith("pending")) ||
                tx.is_grayed_out));
    ...
    const payoutInfo = (tx) => {
        ...
        if (tx.payout_badge === "paid_out")
            return {
                key: "paid",
                label: "Paid out",
                cls: "bg-emerald-50 text-emerald-700 border-emerald-200",
            };
        if (tx.payout_badge === "this_week")
            return {
                key: "in_payout",
                label: "In this payout",
        ...
        return {
            key: "queued",
            label: "Queued",
            cls: "bg-gray-50 text-black/80 border-gray-200",
        };
    };
    ...
    const visible = useMemo(
        () =>
            filter === "all"
                ? transactions
                : transactions.filter((tx) => bucketOf(tx) === filter),
        [transactions, filter],
    );
        ...
        <div className="bg-white rounded-box border border-gray-200 overflow-hidden ">
            <div className="p-5 md:p-6 border-b border-gray-100 flex flex-wrap justify-between items-center gap-3">
                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2 tracking-tight">
                    ...
                    Ledger history{" "}
                    {active_tab === "overview" ? `(${tax_year})` : "(all time)"}
                </h2>
                <Link
                    href={route("financial.history", {
                        year: tax_year?.split("-")[0],
                    })}
                    className="text-[12px] text-[#FF007F] hover:text-[#d83a7c] font-semibold uppercase tracking-wide flex items-center gap-1 group"
                >
                    Full history{" "}
                    <ChevronRightIcon
                        size={14}
                ...
                {FILTERS.map((f) => {
                    ...
                    return (
                        ...
                        >
                            {f.label}
                            <span
                                className={`ml-1.5 tabular-nums ${active ? "text-white/60" : "text-black/60"}`}
                            >
                                {counts[f.key] ?? 0}
                            </span>
                        </button>
                ...
                <table className="w-full text-left">
                    <thead className="bg-gray-50">
                        <tr className="text-black/60 text-[12px] uppercase font-semibold tracking-wide">
                            ...
                            <th className="px-5 md:px-6 py-3.5 text-right">
                                Amount
                            </th>
                            <th className="px-5 md:px-6 py-3.5 text-right">
                                Payout
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {visible.length === 0 ? (
                            <tr>
                                <td
                                    colSpan="4"
                                    className="px-6 py-12 text-center text-black/60 text-sm font-medium"
                                >
                                    Nothing here yet.
                                </td>
                            </tr>
                            ...
                            visible.map((tx) => {
                                ...
                                return (
                                    <tr
                                        key={tx.uuid}
                                        className={`${rowTint(po.key)} hover:bg-gray-50/80 transition-colors`}
                                    >
                                        {/* Supporter + when */}
                                        <td className="px-5 md:px-6 py-4 align-top">
                                            {tx.supporter ? (
                                                <Link
                                                    ...
                                                    className="flex flex-col group/supp"
                                                >
                                                    <span className="text-gray-900 font-semibold capitalize group-hover/supp:text-[#FF007F] transition-colors leading-tight">
                                                        {tx.supporter.name}
                                                    </span>
                                                    <span className="text-[12px] text-black/60 group-hover/supp:text-[#FF007F]/70 transition-colors">
                                                        @{tx.supporter.username}
                                                    </span>
                                                </Link>
                                            ) : (
                                                <div className="flex flex-col">
                                                    <span className="text-gray-900 font-semibold capitalize leading-tight">
                                                        {tx.guest_name ||
                                                            "Guest"}
                                                    </span>
                                                    {!tx.guest_name && (
                                                        <span className="text-black/60 italic text-[12px]">
                                                            Guest user
                                                        </span>
                                                    )}
                                                </div>
                                        ...
                                        <td className="px-5 md:px-6 py-4 align-top min-w-[220px]">
                                            <div className="font-semibold text-gray-900 line-clamp-2 leading-snug">
                                                {tx.description}
                                            </div>
                                            <div className="mt-1.5">
                                                <span
                                                    className={`inline-block whitespace-nowrap text-[12px] px-2 py-0.5 rounded-full font-semibold uppercase tracking-wide border ${
                                                        [
                                                            "digital",
                                                            "instant",
                                                        ].includes(tx.item_type)
                                        ...
                                        <td className="px-5 md:px-6 py-4 text-right align-top min-w-[160px]">
                                            <div
                                                className={`text-[16px] font-bold tabular-nums tracking-tight ${tx.type === "income" ? "text-emerald-600" : "text-red-600"}`}
                                            >
                                                {tx.type === "income"
                                                    ? "+"
                                                    : ""}
                                        ...
                                        <td className="px-5 md:px-6 py-4 text-right align-top">
                                            <div className="flex flex-col items-end gap-1.5">
                                                <span
                                                    className={`px-2.5 py-1 whitespace-nowrap rounded-full text-[12px] font-semibold uppercase tracking-wide border ${po.cls}`}
                                                >
                                                    {po.label}
                                                </span>
                                                {tx.item_status && (
                                                    <span
                                                        className={`px-2 py-0.5 whitespace-nowrap rounded-full text-[12px] font-medium tracking-wide ${
                                                            [
                                                                "complete",
                                                                "delivered",
                                                                "accepted",
                                                            ].some((s) =>
                                                                tx.item_status.endsWith(
                                                                    s,
                                                                ),
                                                            )
                                                                ? "text-emerald-600"
