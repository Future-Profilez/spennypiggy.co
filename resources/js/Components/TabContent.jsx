...
const AboutTab = ({ user, sLinks }) => (
    <div className="p-4 space-y-4">
        {user?.bio ? (
            <div className="bg-white rounded-[30px] md:rounded-[40px]  p-4 shadow-sm">
                <h3 className="font-semibold text-lg mb-2">About</h3>
                <p className="text-gray-700 leading-relaxed">{user.bio}</p>
            </div>
        ) : (
            <div className="bg-gray-50 rounded-[30px] md:rounded-[40px]  p-6 text-center">
        ...
        {sLinks && sLinks.length > 0 && (
            <>
                ...
                <div className="space-y-6">
                    <div className="bg-white/5 backdrop-blur-2xl border border-white/5 rounded-[30px] md:rounded-[40px]  p-8 shadow-2xl transition-all hover:border-white/10 group">
                        ...
                        <div className="flex flex-col gap-4">
                            {sLinks.map((link, index) => (
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center justify-between p-5 bg-white/5 rounded-[30px] md:rounded-[40px]  border border-white/5 text-white/70 hover:bg-white/10 hover:text-white transition-all group/link"
                                >
                                    <span className="font-black text-[10px] tracking-[0.2em] uppercase">
                                        {link.platform}
                                    </span>
                                    <div className="w-10 h-10 flex items-center justify-center rounded-[30px] md:rounded-[40px]  bg-white/5 group-hover/link:bg-[#05EFB8]/20 group-hover/link:text-[#05EFB8] transition-all">
                                        →
                                    </div>
                                </a>
                            ))}
