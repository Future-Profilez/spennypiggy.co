...
import { Shield, LogOut, UserX, Search, X, Monitor, Smartphone, Globe, UserMinus } from 'lucide-react';
...
export default function SecurityZone() {
    ...
    const revokeSession = async (sessionId) => {
        ...
        try {
            await axios.post(route('creator.security.sessions.revoke'), { session_id: sessionId });
    ...
    const handleSearch = async (e) => {
        ...
        setSearching(true);
        try {
            const res = await axios.get(route('creator.security.search-users', { query }));
    ...
    const blockUser = async (userId) => {
        try {
            await axios.post(route('creator.security.block-user'), { user_id: userId });
    ...
    const unblockUser = async (userId) => {
        try {
            await axios.delete(route('creator.security.unblock-user', { id: userId }));
    ...
    if (loading) return <div className="p-8 text-center font-gulfs">LOADING SECURITY DATA...</div>;
        ...
        <div className="space-y-8 p-2 smax-h-[70vh] overflow-y-auto custom-scrollbar">
            <div>
                ...
                <div className="space-y-3">
                    {sessions.map((session) => (
                        <div key={session.id} className="flex items-center justify-between p-4 bg-white border-2 border-black rounded-[20px] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                            <div className="flex items-center gap-4">
                                <div className="p-2.5 bg-pink-100 rounded-xl border border-black text-pink-600">
                                    {session.device.is_mobile ? <Smartphone size={24} strokeWidth={2.5} /> : <Monitor size={24} strokeWidth={2.5} />}
                                </div>
                                <div>
                                    <p className="font-black text-gray-900 flex items-center gap-2">
                                        {session.device.browser} on {session.device.platform}
                                        {session.is_current_device && <span className="text-[10px] bg-green-500 text-white px-2 py-0.5 rounded-full uppercase tracking-tighter">Active Now</span>}
                                    </p>
                                    <p className="text-xs text-gray-500 font-bold uppercase tracking-wide">{session.ip_address} • {session.last_active}</p>
                                </div>
                            </div>
                    ...
                    {sessions.length === 0 && <p className="text-gray-500 italic text-center p-4">No other active sessions found.</p>}
                </div>
            </div>
            ...
            <div>
                ...
                <div className="relative mb-6">
                    <div className={`flex items-center gap-3 p-4 bg-white border-2 border-black rounded-[20px] transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${searchQuery.length >= 2 ? 'ring-2 ring-pink-500' : ''}`}>
                        <Search size={22} className="text-gray-400" strokeWidth={2.5} />
                        <input 
                            type="text" 
                        ...
                        <div className="absolute z-20 w-full mt-3 bg-white border-2 border-black rounded-[25px] shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
                            {searching ? (
                                <div className="p-8 text-center">
                                    ...
                                    <p className="text-xs font-bold uppercase text-gray-400 mt-2">Searching...</p>
                                </div>
                            ) : filteredSearchResults.length > 0 ? (
                                filteredSearchResults.map(user => (
                                    <div key={user.id} className="flex items-center justify-between p-4 hover:bg-pink-50 border-b-2 border-black last:border-none transition-colors">
                                ...
                                <div className="p-8 text-center">
                                    <UserMinus className="mx-auto text-gray-300 mb-2" size={32} />
                                    <p className="text-gray-400 font-bold uppercase text-sm">No users found</p>
                                    <p className="text-gray-300 text-xs mt-1">Try a different username</p>
                                </div>
                            )}
                        </div>
                    )}
                ...
                <div className="space-y-4">
                    {blockedUsers.length > 0 ? (
                        blockedUsers.map((block) => (
                            <div key={block.id} className="flex items-center justify-between p-4 bg-white border-2 border-black rounded-[20px] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                                <div className="flex items-center gap-4">
                                    <img 
                                        src={block.avatar_url || '/default-avatar.png'} 
                                    ...
                                    />
                                    <div>
                                        <p className="font-black text-gray-900 uppercase">{block.name}</p>
                                        <p className="text-xs text-gray-500 font-bold uppercase tracking-tight">@{block.username} • BLOCKED {block.blocked_at}</p>
                                    </div>
                                </div>
                                <button 
                        ...
                        <div className="text-center py-10 bg-gray-50 border-2 border-dashed border-gray-300 rounded-[30px]">
                            ...
                            <p className="text-gray-400 font-bold uppercase text-sm">No blocked users yet</p>
                            <p className="text-gray-300 text-xs mt-1">Search and block users above</p>
                        </div>
                    )}
                </div>
            </div>
