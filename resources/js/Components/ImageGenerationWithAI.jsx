...
export default function ImageGenerationWithAI({ classes, size, update }) {
    ...
    const renerate = () => {
      setGenerating(true);
      axios.post("/dalle-image", { prompt: prompt, size:size || "1024x1024" })
      .then((resp) => {
         setGenerating(false);
         if(resp.data.status){
            successAlert(resp.data.success || "Image generated successfully.");
            setAiImage(resp.data.image_url);
         } else {
            errorAlert(resp.data.message || "Failed to generate image. Please try again.");
         }
      })
      .catch((err) => {
            setGenerating(false);
    ...
    const useThis = () => { 
      setUpdating(true);
      axios.post("/upload-dalle-image", {url: AiImage.url}).then((resp) => {
         setUpdating(false);
         if(resp.data.status){
            setClose(false);
            setAiImage(null);
            ...
            errorAlert(resp.data.message || "Failed to upload image. Please try again.");
         }
      }).catch((err) => {
         setUpdating(false);
         errorAlert(err.response.data.error || "Something went wrong.");
                    ...
                    {generating ? (
                        <div className="pt-4">
                            <div className="spinner ">
                                ...
                                <h2 className="text-center text-lg font-normal">Image Generating...</h2>
                            </div>
                        </div>
                    ) : 
                     <>
                        {AiImage &&  AiImage.url ? 
                           <div className="pt-4">
                              <img className="w-full max-h-[350px] object-cover rounded-xl" src={AiImage.url} alt="image"/> 
                              <button
                              ...
                              <button
                                 ...
                                 onClick={useThis}>
                                {updating ? "Uploading..." :"Use this image"}
                              </button>
                           </div>
                           : 
                           <div className="pt-4">
