...
export default function Notifications() {
  ...
  return (
    ...
    <div className='relative notifications' ref={wrapperRef} >
        ...
        {open ? <div id="toast-notification" className="w-full p-4 text-gray-900 bg-white rounded-[30px] md:rounded-[40px]  shadow " role="alert">
                ...
                <div className={`flex items-start justify-between ${n && n.is_read ? 'read' : 'unread' } my-2 ${ i == lists.length-1 ? '' : "border-gray-200 border-b" } py-3  `}>
                    ...
                    <div className="relative inline-block shrink-0">
                      <Link href={n && n.user && n.user.username || '/'} >
                        <img className="w-12 h-12 rounded-[30px] md:rounded-[40px]  object-cover" src={ n && n?.user && n?.user?.avatar_url || userphoto } alt="spenny piggy user"/>
                      </Link>
                    </div>
                </div>
                </>
