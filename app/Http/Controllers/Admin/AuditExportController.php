...
class AuditExportController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        // 1. Disputes CSV
        $disputes = Dispute::where('created_at', '>=', $cutoff)->get();
        ...
        if (Schema::hasTable('stripe_webhook_status')) {
            $webhooks = StripeWebhookStatus::where('created_at', '>=', $cutoff)
        ...
        // 8. Platform Summary
        $txCount = Payment::where('created_at', '>=', $cutoff)->whereIn('status', ['succeeded', 'review_hold', 'refunded', 'disputed'])->count();
        $disputeCount = Dispute::where('created_at', '>=', $cutoff)->count();
        $refundCount = Payment::where('created_at', '>=', $cutoff)->where('status', 'refunded')->count();
        $summary = [
            [
