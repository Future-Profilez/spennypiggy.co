...
class CheckoutController extends Controller
{
    ...
    public function __construct(UserProfileService $userProfileService, RiskEngineService $riskEngine)
    {
    ...
    public function createCheckout($creator_id, $user_id_or_device = null)
    {
        ...
        try {
            ...
            $payload = [
                ...
                // Session-level metadata (webhook reads this to identify a cart
                // checkout on async bank settlement).
                'metadata' => array_merge((array) $paymentIntentData['metadata'], ['type' => 'cart']),
            ...
            try {
                ...
                $metrics = app(RiskService::class)->recalculateMetrics((string) $owner->uuid);
                $reserveRate = app(ReservePolicy::class)->getEffectiveReservePercent($owner, $metrics, now());
                ...
                Payment::create([
                    ...
                    'amount' => app(MoneyNormalizer::class)->toGbpMinor($rawAmountMinor, (string) strtoupper($chargeCurrency)),
                    'reserve_amount_minor' => app(MoneyNormalizer::class)->toGbpMinor($reserveMinor, (string) strtoupper($chargeCurrency)),
                    'currency' => 'gbp',
    ...
    */
    ...
    public function successCheckout($id)
    {
        ...
        try {
            ...
            // Delayed-settlement bank methods (SEPA/ACH): the debit may still be
            ...
            if (! config('payments.instant_fulfilment', true)
                ...
                try {
                    ...
                    $settled = $liveSession && ($liveSession->payment_status ?? null) === 'paid';
                } catch (\Throwable $e) {
                    Log::error('Cart bank settlement check failed', ['session_id' => $sessionId, 'error' => $e->getMessage()]);
                }
            ...
            foreach ($getdata as $dd) {
                ...
                try {
                    ...
                    // fee-profile-aware gross-up; no recompute needed here.
