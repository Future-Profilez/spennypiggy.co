...
class ShopsController extends Controller
{
    ...
    {
        // Find shop with proper null check
        $shop = Shop::where('uuid', $session_id)->first();
        ...
        // Handle payment session if exists
        if (!empty($session_id)) {
            $payments = ShopPayment::where('session_id', $session_id)->first();
            if ($payments) {
                $opened = $payments->opened ?? 0;
                $payments->opened = 1;
                $payments->save();
            }
        }
        ...
        if (!empty($shop->user) && !empty($shop->user->vat_amount_percentage)) {
            $vat_percentage_amount = round(($amount + $tax) * $shop->user->vat_amount_percentage / 100, 2, PHP_ROUND_HALF_UP);
        }

        // Get card capabilities with null check
        $card_capabilities = false;
        if (!empty($shop->user) && !empty($shop->user->account_id)) {
