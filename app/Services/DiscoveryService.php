...
class DiscoveryService
{
    ...
    {
        $cacheKey = "trending_creators_{$limit}";
    ...
    {
        $cacheKey = "new_verified_creators_{$limit}";
    ...
    public function getSearchCreators($filters, $limit = 24)
    {
        ...
        $cacheKey = "search_creators_{$limit}_{$filterHash}";

        return Cache::remember($cacheKey, $this->getCacheTtl(), function () use ($filters, $limit) {
            $query = User::query()
                ->where('suspended_account', 0)
                ->where('role', 1)
                ->where('profile_status_lock', 2);
    ...
    {
        $filterHash = md5(json_encode($filters));
        $cacheKey = "search_wishes_{$limit}_{$filterHash}";
    ...
    {
        $cacheKey = "top_earners_{$period}_{$limit}";

        return Cache::remember($cacheKey, $this->getCacheTtl(), function () use ($period, $limit) {
            if ($limit < 1) { $limit = 9; }
    ...
    {
        $cacheKey = "featured_wishes_{$limit}";
    ...
    {
        $cacheKey = "featured_bills_{$limit}";
    ...
    {
        $cacheKey = "featured_memberships_{$limit}";
    ...
    {
        $filterHash = md5(json_encode($filters));
        $cacheKey = "search_bills_{$limit}_{$filterHash}";
    ...
    {
        $filterHash = md5(json_encode($filters));
        $cacheKey = "search_memberships_{$limit}_{$filterHash}";
